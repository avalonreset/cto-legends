...
REPOSITORIES = {key: "avalonreset/" + key for key in (
    ...
    "legends-stable-audio-3", "legends-obs-kit", "legends-hyperyap", "legends-empire", "legends-grant", "legends-firecrawl", "legends-yt-dlp", "legends-ambient-intelligence", "legends-captions")}
RECIPES = set(REPOSITORIES)
GUIDED_MODULES = {"legends-hyperyap"}
MODULE_ALIASES = {"hyperyap": "legends-hyperyap", "legends-ultimate-captions": "legends-captions"}
...
def probe(key, release, *, log=False):
    ...
    if key == "legends-dataforseo-kit":
        ...
        check([python, "-c", "from legends_dataforseo import api_request; import importlib.metadata as m; v = tuple(int(x) for x in m.version('legends-dataforseo-kit').split('.')[:3]); assert v >= (0, 1, 0), v"], release)
    elif key == "legends-github":
        check([python, source / "legends_github.py", "capabilities"], release)
        check([python, "-c", "from legends_dataforseo import api_request; import importlib.metadata as m; assert callable(api_request); v = tuple(int(x) for x in m.version('legends-dataforseo-kit').split('.')[:3]); assert v >= (0, 1, 0), v"], release)
    ...
    elif key == "legends-empire":
        check([python, source / "scripts" / "claude-empire.py", "contracts", "--check-only"], source)
        check([python, source / "scripts" / "claude-empire.py", "package", "validate"], source)
    ...
    elif key == "legends-captions":
        check([python, "-c", "from legends_captions import __version__; assert __version__"], release)
        check([python, "-m", "legends_captions", "--help"], release)
    else:
        raise ValueError("No managed probe for this module")


def prepare(key, module, home):
    if key == "legends-empire" and sys.version_info < (3, 11):
        raise ValueError("legends-empire requires Python 3.11 or newer; run cto-legends with a supported Python")
    ...
    if key == "legends-obs-kit":
        ...
        python = python_at(release)
        if key in {"legends-dataforseo-kit", "legends-stable-audio-3", "legends-firecrawl", "legends-yt-dlp", "legends-ambient-intelligence", "legends-captions"}:
...
def status(home):
    ...
    for key, relative in state["active"].items():
        ...
        result[key] = {"version": receipt["version"], "commit": receipt["commit"],
                       ...
                       "guide": str(release / "source" / "skills" / "legends-empire" / "SKILL.md") if key == "legends-empire" else str(release / "source" / "AGENTS.md"), "scope": receipt["scope"]}
    return result
